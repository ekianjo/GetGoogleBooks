...
def makepdf(mypath,title):
	...
	cmd = 'python img2pdf.py {0} -o "BOOKS/{1}.pdf" -C 1'.format(longstring,title)
	print cmd
...
def main(args):
    import optparse
    usage = """usage: GOOGLE_BOOK_OR_ID

    Download a Google Book and create a PNG image for each page.""" 
    a=PyZenity.InfoMessage("This Application will let you download Google Books available in browser mode in PNG format locally for offline reading", timeout=10)
    a=PyZenity.InfoMessage("Note that you need Java to be installed for this application to run", timeout=3)
    bookurl=PyZenity.GetText("Please paste the path of the Google book to download","URL here")
    ...
    for info, page, image_data in download_book(bookurl):
        ...
        if os.path.isdir("BOOKS")==False:
			...
			if os.path.isdir("BOOKS/{0}".format(info["title"])):
				open("BOOKS/{0}/{1}".format(info["title"],output_file), "w").write(image_data)
				print output_file
			else:
				os.mkdir("BOOKS/{0}".format(info["title"]))
				open("BOOKS/{0}/{1}".format(info["title"],output_file), "w").write(image_data)
				print output_file
				
    makepdf("BOOKS/{0}".format(info["title"]),info["title"])
